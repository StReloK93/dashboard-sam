<template>
   <main class="border-r border-zinc-800 flex flex-col xl:flex-grow-0 flex-grow">
      ...
      <aside class="green-scroll overflow-y-auto flex-grow scroll overflow-x-hidden relative grid-col">
         <TransportProcess counter="reys" :data="[]" :grid-cols="`xl:grid-cols-5 lg:grid-cols-5 grid-cols-3`" color="yellow" class="opacity-0" />
         <main class="absolute inset-0">
            <TransportProcess counter="reys" :data="transportStore.inProcess" :grid-cols="`xl:grid-cols-5 lg:grid-cols-5 grid-cols-3`"
               @openModal="(transport) => store.openModal(0, transport)" :title="$t('inprocess')" color="green" />
            <TransportProcess counter="timer" :data="transportStore.inExcavator" :grid-cols="`xl:grid-cols-5 lg:grid-cols-5 grid-cols-3`"
