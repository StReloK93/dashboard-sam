<template>
   <section class="flex flex-col">
      ...
      <main class="flex-grow relative w-full text-sm">
         <aside
            class="absolute inset-0 overflow-x-hidden overflow-y-auto scroll indigo-scroll shadow"
         >
            <template v-for="option in options">
               <div
                  @click="select(option)"
                  :class="{ 'bg-zinc-900': model.includes(option.id) }"
...
const selectedOptions = computed(() => {
   return props.options.filter((option: any) =>
      model.value.includes(option.id),
   );
});

function select(option: any) {
   if (model.value.includes(option.id))
      model.value = model.value.filter((value: any) => value != option.id);
