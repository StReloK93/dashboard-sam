   ...
   >
      <aside @mousedown.stop class="w-[1280px] h-[600px] relative">
         ...
         <div class="slider-item">
            <div class="mb-1.5">
               ...
               >
                  ...
                  <div class="flex gap-1 ml-1 flex-grow">
                     ...
                     <button
                        @click="tab = 2"
                        :class="setColor(tab == 2)"
                        class="xl:w-24 w-20 h-full font-semibold rounded shadow xl:text-base text-xs"
                     >
                        <i class="fa-solid fa-table-list"></i> {{ $t("table") }}
                     </button>
                     <button
                        v-if="props.color == 'orange'"
                        @click="tab = 3"
                        :class="setColor(tab == 3)"
                     ...
                     <button
                        v-if="props.color == 'indigo'"
                        @click="tab = 4"
                        :class="setColor(tab == 4)"
                        class="px-2 h-full font-semibold rounded shadow ml-1 flex-grow xl:text-base text-xs"
                     >
                        <i class="fa-solid fa-table-list"></i>
                        {{ $t("inremontcause") }}
            ...
            <TransportProcessGroupChart
               v-if="selectedCars.length && dataSmena"
               v-show="tab == 1"
            ...
            <TransportProcessGroupOilTable
               ...
               @table-update="(data) => (tableData = data)"
               v-show="tab == 3"
               :selectedCars="selectedCars"
            />
            <TransportProcessGroupCauses
               v-if="selectedCars.length && props.color == 'indigo'"
               @table-update="(data) => (tableData = data)"
               v-show="tab == 4"
               :selectedCars="selectedCars"
               :color="props.color"
...
import TransportProcessGroupOilTable from "@/noname/TransportProcessGroupOilTable.vue";
import TransportProcessGroupCauses from "@/noname/TransportProcessGroupCauses.vue";
