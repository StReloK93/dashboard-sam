<template>
   <section
      @mouseup="$emit('close')"
      class="fixed inset-0 bg-zinc-950/70 flex justify-center items-center z-50"
   >
      <aside @mouseup.stop class="w-[992px] h-[540px] relative">
         <main class="slider-item py-1">
            <div class="px-1">
               ...
               >
                  Avtoag'dargichlarni yoqilg'i olishda kutishga ketgan vaqtlari
                  (Smenalar bo'yicha)
               </div>
...
function changeDate(modelData: any) {
   if (modelData) {
...
function getChartData() {
   useFetch({
      ...
      onLoad: ({ data }: any) => {
         ...
         const chartData = calculateChartDataPrices(
            formatterToExcel(allData.value),
         );
         // @ts-ignore
