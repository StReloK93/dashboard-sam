...
function show(transport_id: number, onload?: Function, formData?: any) {
   return useFetch<any[]>({
...
function saveTime(
   formData: { transport_state_id: number; time: number | string },
   onload?: Function,
) {
   return useFetch<any[]>({
      url: `${baseUrl}/save-time`,
      method: "post",
      formData: formData,
      onLoad: onload,
   });
...
export default { index, clean, show, selectSmena, updateCause, saveTime };
